...
std::string KrigingTypeSelector_accessor::value() const {
  ...
  if( selector_->krigingType().contains( "(LVM)" ) ) {
    result << "grid=\"" << String_Op::qstring2string(selector_->LocalMeanProperty().first) << "\"  "
	   << "property=\"" << String_Op::qstring2string(selector_->LocalMeanProperty().second) << "\" "
	   << "/>" << std::endl
	   << "</" << widget_name << ">" << std::endl;
    return result.str();
  }
...
PropertyRegionSelector_accessor::PropertyRegionSelector_accessor( QWidget* widget ) 
