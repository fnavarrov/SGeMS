...
Slice_display_control::Slice_display_control( const QString& label, 
  ...
  slice_display_toggle_ = new QCheckBox( label, this);
  
  slice_pos_slider_ = new QSlider( Qt::Horizontal, this);
  slice_pos_slider_->setMinimumWidth( 40 );
  slice_pos_slider_->setMaximum( slice_->max_position() );
  if( slice_ )
    slice_pos_slider_->setMaximum( slice_->max_position() );
  else 
    slice_pos_slider_->setMaximum( 0 );

  slice_pos_spinbox_ = new QSpinBox( this);
  slice_pos_spinbox_->setMaximum( slice_->max_position() );
  if( slice_ )
    slice_pos_spinbox_->setMaximum( slice_->max_position() );
  else 
    slice_pos_spinbox_->setMaximum( 0 );
  
  remove_slice_button_ = new QPushButton( "Remove", this);
  //remove_slice_button_->setMaximumWidth( 50 );

  hlayout_ = new QHBoxLayout( this );
  hlayout_->addWidget( slice_display_toggle_ );
  hlayout_->addSpacing( 2 );
...
//================================================================
void Display_pref_panel::sliceUpdated()
{ 
	oinv_desc_->oinv_node()->touch();
	emit renderRequest();
}
Display_pref_panel::Display_pref_panel( Oinv_description* oinv_desc,
  ...
  if( !oinv_strati_desc_ ) {
	...
	BBox_checkbox_->setEnabled(false);
  }
  ...
  for( std::list<std::string>::const_iterator it = properties.begin();
    ...
    property_selector_->addItem( QString(it->c_str()) );
  }
  ...
  transparency_colormap_pix_->setLayout(v);
  transparency_colormap_pix_->layout()->addWidget(transparency_cmap_bitmap);
  */
  ...
  if( !properties.empty() ) {
    ...
    if( current_property.isEmpty() ) {
      property_selector_->setCurrentIndex( 0 );
...
#ifdef TEST
  oinv_strati_desc_->oinv_node()->touch();
#endif
}
...
void Display_pref_panel::set_property( const QString& prop ) {
  QByteArray tmp = prop.toLatin1();
  oinv_desc_->set_property( std::string( tmp.constData() ) );
  ...
  for( ; i < colormap_selector_->count() ; i++ ) {
    if( colormap_selector_->itemText(i) == qcmap_name ) {
      break;
    }
  }
  colormap_selector_->setCurrentIndex( i );
  ...
  if( oinv_strati_desc_ ) {
    QString intervals = 
      oinv_strati_desc_->transparency_intervals( std::string(tmp.constData()) ); 
...
void Display_pref_panel::set_colormap( const QString& map_name ) {
  QByteArray tmp = map_name.toLatin1();
  oinv_desc_->set_colormap( std::string( tmp.constData() ) );
...
void Display_pref_panel::add_slice() {
  int axis_int = slice_axis_selector_->currentIndex();
  Oinv::Axis axis;
...
void Display_pref_panel::add_slice( Oinv::Axis axis ) {
  if( !slices_frame_ ) {
    slices_frame_ = new QFrame(volume_explorer_prefs_);
    QVBoxLayout * vbox = new QVBoxLayout();

    slices_frame_->setLayout(vbox);
    slices_frame_->setFrameStyle( QFrame::Box);
    slices_frame_->setFrameShadow(QFrame::Sunken);
    vbox->setSpacing( 0 );
    slices_frame_->setContentsMargins( 0,0,0,0 );

    /*
    QHBoxLayout* slices_frame_layout_ =
      new QHBoxLayout( volume_explorer_prefs_);
    slices_frame_layout_->setSpacing(-1);
    slices_frame_layout_->addStretch( 0 );
    slices_frame_layout_->addWidget( slices_frame_ );
    */
    
    volume_explorer_prefs_->layout()->addWidget( slices_frame_ );
    ///volume_explorer_prefs_->layout()->addItem( slices_frame_layout_ );
  }
  QString slice_label;
  ...
  QByteArray tmp = slice_name.toLatin1();
  Slice_display_control* new_control = 
    new Slice_display_control( slice_label, 
			       oinv_strati_desc_->slice( slice_id ), slice_id,
			       slices_frame_, tmp.constData() );

  slices_frame_->layout()->addWidget(new_control);
  slice_controls_list_.push_back( new_control );

  if( !slices_frame_->isVisible() )
    slices_frame_->show();

  QObject::connect( new_control, SIGNAL( sliceUpdate(  ) ),
	  this, SLOT( sliceUpdated( ) ) );
...
void Display_pref_panel::change_selected_property( const QString& prop ) {
  ...
  for( ; i < property_selector_->count() ; i++ ) {
    if( property_selector_->itemText( i ) == prop )
      break;
  }
  property_selector_->setCurrentIndex( i );
