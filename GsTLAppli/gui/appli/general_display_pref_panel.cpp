...
General_display_pref_panel( SoQtGsTLViewer* viewer,
  ...
  /*
  QVBoxLayout * vbox = new QVBoxLayout(GroupBox3);
...
void General_display_pref_panel::save_colorbar()
{
  ...
  tmp = object_selector_->selectedGrid().toLatin1();
  std::string grid_name = tmp.constData();
  tmp = object_selector_->selectedProperty().toLatin1();
  std::string property_name = tmp.constData();
  if( property_name.empty() || grid_name.empty() ) return;

  if( !colorbar_root_ ) 
    init_colormap_node();

  QString s = QFileDialog::getSaveFileName( this, "save file dialog");

  tmp = s.toLatin1();
  ofstream of(tmp.constData());
